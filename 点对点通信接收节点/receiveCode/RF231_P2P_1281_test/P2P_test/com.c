...
void com_init( baud_rate_t rate ){
	...
	//Enable external interrupt on FTDI_RX pin.
 ...
 */
	void com_send_hex( uint8_t nmbr ){
			...
			//Wait until the fifo is ready.
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			//*FTDI_Fifo = '0'; //Put symbol in data register.
		
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			//*FTDI_Fifo = 'x'; //Put symbol in data register.
			
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			*FTDI_Fifo = hex_lookup[ ( nmbr >> 4 ) & 0x0F ];
	...
	void com_send_number( uint8_t nmbr ){
			...
			//Wait until the fifo is ready.
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			*FTDI_Fifo = dec_lookup[nmbr/1000];; //Put symbol in data register.
		
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			*FTDI_Fifo = dec_lookup[nmbr/100%10]; //Put symbol in data register.
			
			while((FTDI_TX_MASK & FTDI_PIN) != LOW){;}
			*FTDI_Fifo = dec_lookup[nmbr/10%10];
